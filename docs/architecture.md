...
- Поддержка десяти контекстов (панелей) кнопок, пронумерованных от 0 до 9
...
- 10 контекстов (панелей) кнопок; панель 0 включена по умолчанию
...
**Ответственность:**
- Нормализация списка контекстов (всегда 10 контекстов `0`…`9`)
...
**Назначение:** Переключение между десятью панелями (контекстами) кнопок `0`…`9`.
...
**Использование в системе:** Используется в AppSettings для хранения списка контекстов. Нормализуется ContextStateHelper до 10 контекстов `0`…`9`.
...
- Contexts (List<PanelContext>) — список контекстов (10 контекстов `0`…`9`)
