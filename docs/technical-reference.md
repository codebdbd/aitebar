...
| `QuickNote.aite-note` | Основной переносимый package быстрой заметки; `QuickNote.rtf` создается как export для внешнего редактора |
...
Quick Note сохраняет визуальный документ в переносимом package `QuickNote.aite-note`; команда открытия файла формирует отдельный RTF-export для внешнего редактора. Поддерживаются:
...
- вставка и сохранение изображений.
