...
- Crash reporting: Sentry is used only for dev/support diagnostics and is enabled if environment variable `AITEBAR_SENTRY_DSN` or `SENTRY_DSN` is set on the machine. Without DSN, application works without telemetry; production installer doesn't include crash reporting by default.
