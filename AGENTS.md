...
- При ответах о текущем состоянии реализации отвечать только по текущему коду. Не использовать `README`, `docs`, `CHANGELOG` или ExecPlan как источник истины для фактического поведения, если пользователь явно не просит ответ по документации.
- Перед таким ответом проверять конкретные файлы реализации (`AiteBar/*.cs`, XAML, тесты или другие релевантные исходники) и указывать ссылки на файлы/строки, на которых основан вывод.
- Если код не был проверен, прямо сказать об этом вместо уверенного ответа. Если документация расходится с кодом, считать источником истины код и отдельно отметить расхождение.
